...
async def create_meeting_room(new_room: MeetingRoomCreate,
...
async def get_meeting_room_by_id(
        room_id: int, session: AsyncSession) -> Optional[MeetingRoom]:
    db_room = await session.execute(select(MeetingRoom).where(
